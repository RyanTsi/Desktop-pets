    ...
    : QSystemTrayIcon(parent)
{
    ...
    app_reset_geo = new QAction(tr("重置"));
    AppMsgHandler::getInstance().regSignal
        ("/window/reset_geometry", app_reset_geo, SIGNAL(triggered()));
    menu->addAction(app_reset_geo);

    set_Form=new QAction(tr("界面"));
    connect(set_Form,&QAction::triggered,this,&Tray::showInterForm);
    menu->addAction(set_Form);

    app_exit = new QAction(tr("退出"));
...
void Tray::showInterForm()
{
    qDebug()<<"Open SetForm";
    if(m_setForm!=nullptr)
    {
        m_setForm = new SetInter();
    }
