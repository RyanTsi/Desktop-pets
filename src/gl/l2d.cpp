...
Allocator GLObj_L2d::_cubismAllocator;
...
void GLObj_L2d::onL2dLButtonUp(const QPointF& pt)
{
    ...
    {
        if (LAppDefine::DebugLogEnable)
            l2dPal::PrintLog("[APP]hit area: [%s]", LAppDefine::HitAreaNameHead);
        _model->SetRandomExpression();
    }
    else if (_model->HitTest(LAppDefine::HitAreaNameBody, x, y))
    {
        if (LAppDefine::DebugLogEnable)
            l2dPal::PrintLog("[APP]hit area: [%s]", LAppDefine::HitAreaNameBody);
        _model->StartRandomMotion(LAppDefine::MotionGroupTapBody, LAppDefine::PriorityNormal);
    }
    flag_mouse_pressed = false;
}
