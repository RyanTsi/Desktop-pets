...
TexManager::TextureInfo*
TexManager::CreateTextureFromPngFile(std::string fileName)
{
    ...
    TexManager::TextureInfo* textureInfo = new TexManager::TextureInfo();
...
void
TexManager::ReleaseTexture(Csm::csmUint32 textureId)
{
    for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
    {
...
void
TexManager::ReleaseTexture(std::string fileName)
{
    for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
    {
        if (_textures[i]->fileName == fileName)
...
TexManager::TextureInfo*
TexManager::GetTextureInfoById(GLuint textureId) const
{
    for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
        if (_textures[i]->id == textureId)
            return _textures[i];
